...
public class week8 {
	...
	public static void main(String[] args) throws Exception{
		try {
			...
			System.out.println("# java Client updated 21-April, 2021 @ MQ");
			...
			while(!rcvd.contains("NONE")){
				if(rcvd.length() > 0)
					if(rcvd.contains("JOBN")){
						job = rcvd.split(" ");
						send("GETS Capable " + job[4] + " " + job[5] + " " + job[6], sock, isNewline);
					} else if(rcvd.equals(".")){
						send("SCHD " + counter + " " + capableServer + " 0", sock, isNewline);
						counter++;
					} else if(rcvd.contains("DATA")){
